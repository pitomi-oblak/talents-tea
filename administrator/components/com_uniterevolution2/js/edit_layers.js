		...
		jQuery("#layeranimeditor_wrap").dialog({
			...
			buttons:{
				...
				Delete: function() {
					...
					if(isOriginal){
						alert("Default11 animations can't be deleted");
					}else{
	...
	t.updateLayerFromFields = function(){
		...
		switch(type){
			...
			case 'image':
				...
				TweenLite.set(jQuery('#slide_layer_' + selectedLayerSerial),{rotationZ:rotation,transformOrigin:parseFloat(objLayer['2d_origin_x'])+'% '+parseFloat(objLayer['2d_origin_y']+'%')});
			break;
		}
	...
	var showHideTimeines = function() {
		...
		jQuery('#button_sort_timing').click(function() {
			...
			if (bst.hasClass("off")) {
				...
				TweenLite.to(jQuery('#sortlist .timeline'),0.5,{autoAlpha:0.5,overwrite:"auto"});
			} else {
				TweenLite.to(jQuery('#sortlist .timeline'),0.5,{autoAlpha:0,overwrite:"auto"});
				bst.addClass("off");				
