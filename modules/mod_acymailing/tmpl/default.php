...
?>
<div id="news-letter" class="text-center parallax-section btacymailing_module<?php echo $params->get('moduleclass_sfx')?>" id="acymailing_module_<?php echo $formName; ?>">
<?php
	$style = array();
	if($params->get('effect','normal') == 'mootools-slide'){
		if(!empty($mootoolsIntro)) echo '<p class="acymailing_mootoolsintro">'.$mootoolsIntro.'</p>'; ?>
		<div class="parallax-content" id="acymailing_toggle_<?php echo $formName; ?>" >
	...
	<div class="container" <?php echo $styleString; ?> >
		...
		<div class="col-sm-8 col-sm-offset-2">
			<?php if(!empty($introText)) echo '<div class="text-center wow fadeInDown" data-wow-duration="700ms" data-wow-delay="300ms"><h1>'.$module->title.'</h1>'.$introText.'</div>';?>
			<?php if(!empty($visibleListsArray) && $listPosition == 'before'){
				...
				<?php }else{?>
			<table class="news-letter wow fadeInUp" data-wow-duration="700ms" data-wow-delay="500ms">
				<?php foreach($visibleListsArray as $myListId){
			...
			<?php }//endif dropdown
				...
				?>
			<table id="newsletter" style="width: 100%;" class="news-letter wow fadeInUp" data-wow-duration="700ms" data-wow-delay="500ms">
				<tr>
					<?php foreach($fieldsToDisplay as $oneField){
						if($oneField == 'name' AND empty($extraFields[$oneField])){
							...
							<td class="acyfield_<?php echo $oneField; ?>">
								<input style="width: 100%;" id="user_email_<?php echo $formName; ?>" <?php if(!empty($identifiedUser->userid)) echo 'disabled="disabled" ';  if(!$displayOutside){ ?> onfocus="if(this.value == '<?php echo $emailCaption;?>') this.value = '';" onblur="if(this.value=='') this.value='<?php echo $emailCaption?>';"<?php } ?> class="inputbox newsletter-email" type="text" name="user[email]" style="width:<?php echo $fieldsize; ?>" value="<?php if(!empty($identifiedUser->userid)) echo $identifiedUser->email; elseif(!$displayOutside) echo $emailCaption; ?>" />
							</td> <?php
